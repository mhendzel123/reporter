...
public class WorkbookLoader {
    ...
    public List<Pracownik> listaPracownikowZFolderu(File folder, String employeeFilter) {
        try {
            for (File fileEntry : folder.listFiles()) {
                if (fileEntry.isDirectory()) {
                    listaPracownikowZFolderu(fileEntry, employeeFilter);
                } else {
                    String nazwaNowegoPracownika = fileEntry.getName().split(".xls")[0].replace("_", " ");
                    if (employeeFilter == "" || nazwaNowegoPracownika.toLowerCase().contains(employeeFilter)) {
    ...
    public List<Projekt> listaProjektowZFolderu(File folder, String projectFilter) {
        try {
            for (File fileEntry : folder.listFiles()) {
                // System.out.println(fileEntry);
                if (fileEntry.isDirectory()) {
                    listaProjektowZFolderu(fileEntry, projectFilter);
                } else {
                    Workbook nowyArkusz = WorkbookFactory.create(fileEntry);
                    for (Sheet sheet : nowyArkusz) {
                        Projekt newProject = new Projekt(sheet);
                        if (projectFilter == "" || newProject.getNazwa().toLowerCase().contains(projectFilter)) {
